...
public extension SCKViewController {
    ...
    internal final class _SCKObjCEventManagingProxy: SCKEventManaging {
        ...
        func scheduleController(_ c: SCKViewController, didSelectEvent event: SCKEvent) {
        ...
        func scheduleController(_ c: SCKViewController, didDoubleClickBlankDate date: Date) {
            delegate?.scheduleController?(c, didDoubleClickBlankDate: date)
        }
        
        func scheduleController(_ c: SCKViewController, didDoubleClickEvent event: SCKEvent) {
            delegate?.scheduleController?(c, didDoubleClickEvent: event)
        }
        
        func scheduleController(_ c: SCKViewController, shouldChangeDurationOfEvent e: SCKEvent,
                                from o: Int, to n: Int) -> Bool {
            return delegate?.scheduleController?(c, shouldChangeDurationOfEvent: e, from: o, to: n) ?? true
        }
        
        func scheduleController(_ c: SCKViewController, shouldChangeDateOfEvent e: SCKEvent,
                                from o: Date, to n: Date) -> Bool {
            return delegate?.scheduleController?(c, shouldChangeDateOfEvent: e, from: o, to: n) ?? true
        }
        
        func scheduleController(_ c: SCKViewController, menuForEvent event: SCKEvent) -> NSMenu? {
            return delegate?.scheduleController?(c, menuForEvent: event)
        }
    }
    
    @available(*,unavailable)
    @objc public func useDayMode() {
        self.mode = .day
    }
    
    @available(*,unavailable)
    @objc public func useWeekMode() {
        self.mode = .week
    }
